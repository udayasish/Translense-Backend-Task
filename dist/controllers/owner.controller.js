...
export const createOwner = asyncHandler(async (req, res) => {
    ...
    if ([fullName, country, state, city, address, email, mobile].some((field) => !field?.trim())) {
    ...
    const emailResponse = await sendVerificationEmail(fullName, email, ownerVerifyCode);
...
export const verifyOwnerByEmail = asyncHandler(async (req, res) => {
    try {
